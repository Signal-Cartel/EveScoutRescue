...
class RescueSuccess {
	...
	public function generateTenderThanksMail($systemName, $sowPilot, $cacheTenders, $coordinator)
	{
...
";
	...
	$coordinator = ($request['closeagent']) ? Output::htmlEncodeString($request['closeagent']) : "Angel Lafisques, Co-ordinator";
	...
	if (empty($cacheParticipants)) {
		...
		foreach ($cacheParticipants as $activity) {
			if (empty($sowPilot)) {
				...
				$cacheTenders = $cacheTenders . EVEMAIL_DELIMITER . Output::htmlEncodeString($activity['Pilot']);
			}
		}
	...
	$tenderThanksMail = $rescueSuccess->generateTenderThanksMail($systemName, $sowPilot, $cacheTenders, $coordinator);
